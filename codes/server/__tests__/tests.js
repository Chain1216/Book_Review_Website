...
test("GET /authors no pagination", async () => {
  await supertest(app).get("/authors/D1")
    .expect(200)
    .then((response) => {
...
test("GET /search/authors basic 1", async () => {
  await supertest(app).get("/search/authors?Home=Bay&Away=Hertha")
    .expect(200)
    .then((response) => {
      expect(response.body.results.length).toEqual(1)
